...
## 💾 รายการซอฟต์แวร์ แพลตฟอร์ม และไลบรารีที่เกี่ยวข้อง (Software, Libraries & Platforms)

โครงงานเครื่องเกลี่ยข้าวอัจฉริยะระบบไร้มือสัมผัสนี้ ผสานสองฝั่งเทคโนโลยีคือ ฝั่งสมองกลฝังตัว (Embedded Hardware) และฝั่งระบบจัดการหลังบ้านผ่านหน้าเว็บพอร์ทัล (Web Dashboard) โดยขับเคลื่อนด้วยซอฟต์แวร์และเทคโนโลยีดังนี้:

### 1. เครื่องมือและซอฟต์แวร์พัฒนาหลัก (Development Tools & Cloud Platforms)
*   **Arduino IDE หรือ VS Code (PlatformIO):** ซอฟต์แวร์เขียนโปรแกรมด้วยภาษา C/C++ สำหรับอัปโหลดโค้ดสั่งงานบอร์ด ESP32
*   **Node.js (เวอร์ชัน 18+):** สภาพแวดล้อมรันไทม์ฝั่งเซิร์ฟเวอร์หลักที่ใช้ขับเคลื่อนระบบร้านค้าและระบบคลัง UltraStore
*   **MongoDB Atlas Cloud Database:** ระบบฐานข้อมูลแบบ NoSQL จัดเก็บสถานะอะไหล่ อุปกรณ์ บัญชีผู้ใช้ สมาชิก และรายการสั่งซื้อ
*   **Vercel Blob Storage:** แพลตฟอร์มคลาวด์สำหรับจัดเก็บไฟล์ภาพชิ้นส่วนอุปกรณ์ รูปภาพสลิปใบเสร็จ และไฟล์มีเดียต่างๆ

### 2. ไลบรารีฝั่งสมองกลฝังตัว (ESP32 Microcontroller Libraries)
*   **`WiFi.h` & `HTTPClient.h`:** สำหรับให้บอร์ด ESP32 เชื่อมต่ออินเทอร์เน็ตลานตากข้าว และส่งสัญญาณเข้า Web API แบบเรียลไทม์
*   **`MPU6050_light.h` & `Wire.h`:** ใช้สื่อสารแบบ I2C กับเซ็นเซอร์ไจโรสโคป เพื่อประมวลผลมุมเอียงและการหมุนตัวของตัวรถ
*   **`HCSR04.h` หรือ `NewPing.h`:** ใช้คำนวณส่งคลื่นความถี่สูงเพื่อหาค่าระยะห่างสิ่งกีดขวางแบบแม่นยำสูง
*   **`ArduinoJson.h`:** สำหรับแปลงแพ็กเกจข้อมูลระหว่าง ESP32 และเซิร์ฟเวอร์ในรูปแบบ JSON (เช่น ส่งรายงานสถานะแบตเตอรี่ หรือรับคำสั่งควบคุมมอเตอร์)

### 3. ไลบรารีฝั่งระบบเว็บจัดการและหน้าร้าน (UltraStore Technical Web Stack)
*   **Next.js 16 (React 19 & TypeScript):** เฟรมเวิร์กสร้างเว็บแอปพลิเคชันหลัก ทั้งหน้าติดต่อลูกค้า, หน้าร้านค้า และหน้าหลังบ้าน Admin
*   **Mongoose (MongoDB ODM):** ไลบรารีเชื่อมต่อและจัดระเบียบโมเดลข้อมูลของสินค้า คลัง และสมาชิกในโปรเจกต์
*   **Zustand (State Management):** ใช้จัดเก็บและจัดการข้อมูลตะกร้าสินค้าชิ้นส่วนอะไหล่แบบเรียลไทม์
*   **Framer Motion:** ห้องสมุดแอนิเมชันสำหรับเอฟเฟกต์ทรานซิชันที่นุ่มนวล หรูหรา ดึงดูดสายตาทุกการตอบสนอง
*   **Tesseract.js (OCR Service):** ซอฟต์แวร์ช่วยสแกนและแกะข้อมูลบนสลิปธนาคารที่อัปโหลดเข้าหน้าเว็บเพื่อตรวจยอดจ่ายอัตโนมัติ
*   **Recharts:** เครื่องมือสร้างกราฟิกแผนภูมิแสดงสถิติวิเคราะห์ยอดขายและระดับสต็อกอะไหล่ในหน้าแผงควบคุมแอดมิน
*   **BcryptJS & JSONWebToken (JWT):** แพลตฟอร์มความปลอดภัยสำหรับการยืนยันตัวตนและการเข้าถึงหน้าจัดการคลังสินค้า
